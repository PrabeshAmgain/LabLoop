...
const SkeletonLoader = () => (
  <div className="w-full max-w-5xl mx-auto space-y-8 animate-fade-in">
    {/* Banner Skeleton */}
    <div className="bg-slate-800/50 border border-slate-700 rounded-2xl p-8 flex items-center gap-8 shadow-xl">
      <div className="w-16 h-16 bg-slate-700/50 rounded-full animate-pulse shrink-0"></div>
      <div className="flex-1 space-y-3">
        <div className="w-32 h-4 bg-slate-700/50 rounded animate-pulse"></div>
        <div className="w-64 h-8 bg-slate-700/50 rounded animate-pulse"></div>
        <div className="w-full h-4 bg-slate-700/50 rounded animate-pulse"></div>
      </div>
    </div>
    {/* Cards Skeleton */}
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      {[1, 2, 3].map(i => (
        <div key={i} className="bg-slate-800/50 border border-slate-700 rounded-xl p-6 h-64 space-y-4">
          ...
          {[1, 2, 3, 4].map(j => (
            <div key={j} className="space-y-2">
              <div className="flex justify-between">
                ...
                <div className="w-8 h-3 bg-slate-700/50 rounded animate-pulse"></div>
              </div>
              <div className="w-full h-2 bg-slate-700/50 rounded-full animate-pulse"></div>
            </div>
          ))}
        </div>
      ))}
...
export const ResultsView: React.FC<ResultsViewProps> = ({ plan, loading = false }) => {
  ...
  const winner = useMemo(() => {
    if (!plan) return null;
    return plan.experiments.find(e => e.id === plan.recommendedWinnerId) || plan.experiments[0];
  }, [plan]);

  const maxLatency = useMemo(() => plan ? Math.max(...plan.experiments.map(e => e.simulatedMetrics.latencyMs)) : 0, [plan]);
  const maxSize = useMemo(() => plan ? Math.max(...plan.experiments.map(e => e.simulatedMetrics.modelSizeMb)) : 0, [plan]);
    ...
    return [...plan.experiments]
      ...
      .sort((a, b) => {
        ...
        switch (sortField) {
          ...
          case 'accuracy': valA = a.simulatedMetrics.accuracy; valB = b.simulatedMetrics.accuracy; break;
          case 'latency': valA = a.simulatedMetrics.latencyMs; valB = b.simulatedMetrics.latencyMs; break;
          case 'size': valA = a.simulatedMetrics.modelSizeMb; valB = b.simulatedMetrics.modelSizeMb; break;
        }
        if (valA < valB) return sortDirection === 'asc' ? -1 : 1;
        if (valA > valB) return sortDirection === 'asc' ? 1 : -1;
        return 0;
      });
  }, [plan, sortField, sortDirection, minAccuracy]);

  if (loading || !plan || !winner) return <SkeletonLoader />;
  ...
  const handleExport = async (model: ExperimentModel, format: ExportFormat) => {
    ...
    switch (format) {
      case 'onnx': extension = 'onnx'; header = new Uint8Array([0x08, 0x01, 0x12, 0x04, 0x4F, 0x4E, 0x4E, 0x58]); break;
      case 'tflite': extension = 'tflite'; header = new Uint8Array([0x54, 0x46, 0x4C, 0x33, 0x00, 0x00, 0x12, 0x00]); break;
      case 'weights': extension = 'bin'; header = new Uint8Array([0xDE, 0xAD, 0xBE, 0xEF, 0x01, 0x00, 0x00, 0x00]); break;
    }
    ...
    setIsPackaging(null);
    setActiveExportId(null);
    setToast({ message: `${model.name} exported as ${format.toUpperCase()}`, type: 'success' });
  };
                    ...
                    return (
                        <div key={exp.id}>
                            ...
                            <div className="w-full bg-slate-700/30 rounded-full h-2.5 overflow-hidden">
                                <div className={`h-full rounded-full transition-all duration-1000 ${isWinner ? 'bg-gradient-to-r from-emerald-500 to-teal-400' : 'bg-gradient-to-r from-blue-600 to-blue-400'}`} style={{ width: `${pct}%` }}></div>
                            </div>
                        </div>
                    );
                })}
                ...
                {plan.experiments.map(exp => {
                    ...
                    return (
                        <div key={exp.id}>
                            ...
                            <div className="w-full bg-slate-700/30 rounded-full h-2.5 overflow-hidden">
                                <div className={`h-full rounded-full transition-all duration-1000 ${isWinner ? 'bg-gradient-to-r from-emerald-500 to-teal-400' : 'bg-gradient-to-r from-amber-600 to-amber-400'}`} style={{ width: `${widthPct}%` }}></div>
                            </div>
                        </div>
                    );
                })}
                ...
                {plan.experiments.map(exp => {
                    ...
                    return (
                        <div key={exp.id}>
                            ...
                            <div className="w-full bg-slate-700/30 rounded-full h-2.5 overflow-hidden">
                                <div className={`h-full rounded-full transition-all duration-1000 ${isWinner ? 'bg-gradient-to-r from-emerald-500 to-teal-400' : 'bg-gradient-to-r from-purple-600 to-purple-400'}`} style={{ width: `${widthPct}%` }}></div>
                            </div>
                        </div>
                    );
                })}
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-4 text-sm">
                    <div className="flex items-center gap-2 text-slate-400">
                        ...
                        <input type="range" min="0" max="100" value={minAccuracy} onChange={(e) => setMinAccuracy(parseInt(e.target.value))} className="accent-blue-500 w-24 h-1 bg-slate-700 rounded-lg appearance-none cursor-pointer" />
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-900/50 text-slate-400 text-xs uppercase tracking-wider select-none">
                <th className="p-4 font-semibold cursor-pointer hover:text-white transition-colors group" onClick={() => handleSort('name')}>
                  <div className="flex items-center gap-1">Model Name <SortIcon field="name" /></div>
                </th>
                <th className="p-4 font-semibold text-right cursor-pointer hover:text-white transition-colors group" onClick={() => handleSort('accuracy')}>
                  <div className="flex items-center justify-end gap-1">Accuracy <MetricTooltip title={METRIC_INFO.accuracy.title} content={METRIC_INFO.accuracy.description} /> <SortIcon field="accuracy" /></div>
                </th>
                <th className="p-4 font-semibold text-right cursor-pointer hover:text-white transition-colors group" onClick={() => handleSort('latency')}>
                  <div className="flex items-center justify-end gap-1">Latency <MetricTooltip title={METRIC_INFO.latency.title} content={METRIC_INFO.latency.description} /> <SortIcon field="latency" /></div>
                </th>
                <th className="p-4 font-semibold text-right cursor-pointer hover:text-white transition-colors group" onClick={() => handleSort('size')}>
                  <div className="flex items-center justify-end gap-1">Size <MetricTooltip title={METRIC_INFO.size.title} content={METRIC_INFO.size.description} /> <SortIcon field="size" /></div>
                </th>
            ...
            <tbody className="divide-y divide-slate-700/50">
              {filteredAndSortedExperiments.length > 0 ? (
                    ...
                    <tr key={exp.id} className={`hover:bg-slate-700/30 transition-colors ${isWinner ? 'bg-emerald-900/10' : ''}`}>
                      ...
                      <td className="p-4 text-center">
                          <div className="relative inline-block" ref={isExportActive ? dropdownRef : null}>
                            <button onClick={() => setActiveExportId(isExportActive ? null : exp.id)} className={`p-2 rounded-lg transition-all border flex items-center gap-1 group ${isExportActive ? 'bg-blue-600 text-white border-blue-500' : 'bg-slate-700/50 text-slate-400 hover:text-blue-400 border-slate-600 hover:border-blue-500/50'}`}>
                            ...
                            {isExportActive && (
                              <div className="absolute right-0 top-full mt-2 w-48 bg-slate-800/95 backdrop-blur-xl border border-slate-600 rounded-xl shadow-2xl z-[60] overflow-hidden animate-fade-in-down origin-top-right">
                                ...
                                <button disabled={!!isPackaging} onClick={() => handleExport(exp, 'weights')} className="w-full px-4 py-2.5 text-left text-xs text-slate-300 hover:bg-slate-700/50 flex items-center gap-3 transition-colors border-l-2 border-transparent hover:border-blue-500 disabled:opacity-50">
                                ...
                                <button disabled={!!isPackaging} onClick={() => handleExport(exp, 'onnx')} className="w-full px-4 py-2.5 text-left text-xs text-slate-300 hover:bg-slate-700/50 flex items-center gap-3 transition-colors border-l-2 border-transparent hover:border-emerald-500 disabled:opacity-50">
                                ...
                                <button disabled={!!isPackaging} onClick={() => handleExport(exp, 'tflite')} className="w-full px-4 py-2.5 text-left text-xs text-slate-300 hover:bg-slate-700/50 flex items-center gap-3 transition-colors border-l-2 border-transparent hover:border-amber-500 disabled:opacity-50">
                ...
                <tr><td colSpan={5} className="p-12 text-center text-slate-500 italic">No experiments matching the current filters.</td></tr>
              )}
            </tbody>
          </table>
        </div>
